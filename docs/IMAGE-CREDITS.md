...
| `benefit-one-point-of-contact.png` | "One point of contact" card on `/services/shipping` | Rendered illustration. Supplied per-benefit via the `image` visual kind, which replaces the generated diagram. |
